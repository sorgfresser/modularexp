...
def histogram_from_counts(counts, bins) -> go.Figure:
    names = ["a (log uniform)", "b (log uniform)", "c (uniform)", "d (uniform)"]
    overall_labels = []
    overall_hist = []
...
# D
counts_d = counts[3, :counts[3].nonzero()[-1] + 1]
bins_d = torch.tensor([19.5, 39.5, 59.5, 79.5], dtype=torch.float) / 100 * counts_d.shape[0]

counts_a = counts[0]
bins_a = torch.tensor([0.0009999999, 0.009999999, 0.099999999, 0.999999999, 9.99999999], dtype=torch.float) / 100 * counts_a.shape[0]
counts_b = counts[1]
bins_b = torch.tensor([0.0009999999, 0.009999999, 0.099999999, 0.999999999, 9.99999999], dtype=torch.float) / 100 * counts_b.shape[0]
counts_c = counts[2, :counts[2].nonzero()[-1] + 1]
